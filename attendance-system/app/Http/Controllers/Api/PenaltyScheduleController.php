...
class PenaltyScheduleController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'applicable_to' => 'required|string|max:20',
            'effective_date' => 'required|date',
        ]);
        if ($validator->fails()) return response()->json(['errors' => $validator->errors()], 422);
        try { $record = AttendancePenaltySchedule::create($validator->validated()); return response()->json(['data' => $record, 'message' => 'Penalty schedule created successfully.'], 201); }
        catch (\Exception $e) { return response()->json(['message' => 'Failed to create penalty schedule.', 'error' => $e->getMessage()], 500); }
    }

    public function show(Request $request, $id): JsonResponse
    {
        $record = AttendancePenaltySchedule::with($this->parseIncludes($request, []))->find($id);
        if (!$record) return response()->json(['message' => 'Penalty schedule not found.'], 404);
    ...
    public function update(Request $request, $id): JsonResponse
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'applicable_to' => 'sometimes|required|string|max:20',
            'effective_date' => 'sometimes|required|date',
            'is_active' => 'boolean',
        ]);
        if ($validator->fails()) return response()->json(['errors' => $validator->errors()], 422);
        try { $record->update($validator->validated()); return response()->json(['data' => $record, 'message' => 'Penalty schedule updated successfully.']); }
        catch (\Exception $e) { return response()->json(['message' => 'Failed to update penalty schedule.', 'error' => $e->getMessage()], 500); }
    }
    ...
    public function destroy($id): JsonResponse
    {
        ...
        catch (\Exception $e) { return response()->json(['message' => 'Failed to delete penalty schedule.', 'error' => $e->getMessage()], 500); }
    }
