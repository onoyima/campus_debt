...
export default function VenuesIndex() {
  ...
  const fetchVenues = (trashed = showTrashed) => {
    setLoading(true)
  ...
  const handleDelete = async (venue) => {
    ...
    setActionLoading(prev => ({ ...prev, [`delete-${venue.id}`]: true }))
    try {
      ...
      setActionLoading(prev => ({ ...prev, [`delete-${venue.id}`]: false }))
    }
  }

  const handleRestore = async (id) => {
    if (!confirm('Restore this venue?')) return
    setActionLoading(prev => ({ ...prev, [`restore-${id}`]: true }))
    try {
      await api.post(`/venues/${id}/restore`)
      fetchVenues(showTrashed)
    } catch (err) {
  ...
  const handleForceDelete = async (id) => {
    ...
    try {
      await api.delete(`/venues/${id}/force`)
      fetchVenues(showTrashed)
    } catch (err) {
