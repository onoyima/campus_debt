...
export default function StaffClockingsIndex() {
  ...
  const fetchData = (trashed = showTrashed) => {
    ...
    api.get(`/staff-clockings?${params}`)
      .then((res) => {
  ...
  const handleRestore = async (id) => {
    if (!confirm('Restore this clocking record?')) return
    setActionLoading(prev => ({ ...prev, [`restore-${id}`]: true }))
    try {
      await api.post(`/staff-clockings/${id}/restore`)
      fetchData()
  ...
  const handleForceDelete = async (id) => {
    if (!confirm('Permanently delete this clocking record? This cannot be undone.')) return
    setActionLoading(prev => ({ ...prev, [`force-${id}`]: true }))
    try {
      await api.delete(`/staff-clockings/${id}/force`)
      fetchData()
    } catch (e) { console.error(e) } finally {
