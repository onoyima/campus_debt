...
export default function PenaltiesIndex() {
  ...
  const fetchData = (trashed = false) => {
    setLoading(true)
    const params = { page, per_page: '15' }
    if (trashed) params.trashed = '1'
    if (search) params.search = search
    api.get('/penalty-schedule', { params })
      .then((res) => {
  ...
  const handleRestore = async (id) => {
    ...
    try {
      await api.post(`/penalty-schedule/${id}/restore`)
  ...
  const handleForceDelete = async (id) => {
    ...
    try {
      await api.delete(`/penalty-schedule/${id}/force`)
      fetchData(showTrashed)
    } catch (e) { console.error(e) } finally {
  ...
  return (
    <AppLayout>
      ...
      <div className="bg-white rounded-lg shadow">
        <Table columns={columns} data={data} loading={loading} onEdit={showTrashed ? undefined : (row) => window.location.href = `/penalties/${row.id}/edit`} onDelete={showTrashed ? undefined : handleDelete} />
